...
use aptos_types::transaction::{SignedTransaction, TransactionRegister};
use futures::{
    channel::{
...
impl DirectMempoolQuorumStore {
    ...
    async fn pull_internal(
    ...
    ) -> Result<TransactionRegister<SignedTransaction>, anyhow::Error> {
        ...
        match monitor!(
            ...
            Ok(resp) => match resp.map_err(anyhow::Error::from)?? {
                QuorumStoreResponse::GetBatchResponse(txns, gas_estimates, dependency_graph) => Ok(TransactionRegister::new(txns, gas_estimates, dependency_graph)),
                _ => Err(anyhow::anyhow!(
    ...
    async fn handle_block_request(
        ...
        let (txns, result) = match self.pull_internal(max_txns, max_bytes, exclude_txns).await {
            Err(_) => {
                ...
                (TransactionRegister::empty(), counters::REQUEST_FAIL_LABEL)
            },
