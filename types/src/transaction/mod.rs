...
impl<T: Send + Sync + Clone> TransactionRegister<T> {
    ...
    pub fn dependency_graph(&self) -> &Vec<Vec<u16>> {
...
pub static RAYON_EXEC_POOL: Lazy<rayon::ThreadPool> = Lazy::new(|| {
    rayon::ThreadPoolBuilder::new()
        .num_threads(cmp::min(64, num_cpus::get()))
        .thread_name(|index| format!("par_exec_{}", index))
        .build()
        .unwrap()
});

pub static RAYON_EXEC_POOL2: Lazy<rayon::ThreadPool> = Lazy::new(|| {
    rayon::ThreadPoolBuilder::new()
        .num_threads(cmp::min(4, num_cpus::get()))
        .thread_name(|index| format!("par_exec_{}", index))
        .build()
        .unwrap()
});
