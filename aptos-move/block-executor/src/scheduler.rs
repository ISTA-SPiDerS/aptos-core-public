...
impl Scheduler {
    ...
    pub fn next_task(&self, committing: bool, profiler: &mut Profiler) -> SchedulerTask {
        loop {
            ...
            if idx_to_validate < idx_to_execute {
                ...
                self.try_execute_next_version(profiler)
            {
    ...
    pub fn finish_abort(&self, txn_idx: TxnIndex, incarnation: Incarnation, profiler: &mut Profiler) -> SchedulerTask {
        ...
        if self.execution_idx.load(Ordering::Acquire) > txn_idx {
            ...
            if let Some((new_incarnation, maybe_condvar)) = self.try_incarnate(txn_idx, profiler) {
...
impl Scheduler {
    ...
    fn try_incarnate<'a>(&self, txn_idx: TxnIndex, profiler: &mut Profiler) -> Option<(Incarnation, Option<DependencyCondvar>)> {
    ...
    /// - Otherwise, return None.
    fn try_execute_next_version(&self, profiler: &mut Profiler) -> Option<(Version, Option<DependencyCondvar>)> {
        ...
        self.try_incarnate(idx_to_execute, profiler)
